...
#     blocks, so it is chosen for redistribution. Starting with the next bank
...
#     back. The final result looks like this: 2 4 1 2. Next, the second bank is
#     chosen because it contains the most blocks (four). Because there are four
#     memory banks, each gets one block. The result is: 3 1 2 3. Now, there is a
#     tie between the first and fourth memory banks, both of which have three
#     blocks. The first bank wins the tie, and its three blocks are distributed
#     evenly over the other three banks, leaving it with none: 0 2 3 4. The
#     fourth bank is chosen, and its four blocks are distributed such that each
...
def reallocations(input)
  ...
  loop do
    ...
    break if seen.include?(redistributed.join('-'))

    seen << redistributed.join('-')
  end

  steps
end

puts reallocations('0 2 7 0')
puts reallocations('5 1 10 0 1 7 13 14 3 12 8 10 7 12 0 6')
