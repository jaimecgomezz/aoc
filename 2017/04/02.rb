...
def are_anagrams(aword, bword)
  aword.split('').sort == bword.split('').sort
end

def passphrases(input)
  valids = input.split("\n").select do |line|
    words = line.split(' ')

    is_valid = true
    for cindex in 0...(words.size - 1) do
      cword = words[cindex]

      for nindex in (cindex + 1)...words.size do
        nword = words[nindex]

        is_valid &&= !are_anagrams(cword, nword)
      end
    end
...
puts passphrases('abcde fghij')
puts passphrases('abcde xyz ecdab')
puts passphrases('a ab abc abd abf abj')
puts passphrases('iiii oiii ooii oooi oooo')
puts passphrases('oiii ioii iioi iiio')
puts passphrases <<~INPUT
