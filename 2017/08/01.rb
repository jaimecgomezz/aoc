...
def condition_met?(condition, register_values)
  register, condition, threshold = condition.split ' '
...
def registers(input)
  register_values = Hash.new { |h, k| h[k] = 0 }

  input.split("\n").each do |line|
    instruction, condition = line.split ' if '

    register, action, amount = instruction.split ' '

    next unless condition_met? condition, register_values
  ...
  register_values.values.max
end

puts registers <<~INPUT
  b inc 5 if a > 1
  a inc 1 if b < 5
  c dec -10 if a >= 1
  c inc -20 if c == 10
